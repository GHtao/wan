...
public class SupportFragmentDelegate {
    ...
    public void onDetach(){
        //gt 防止fragment返回的时候 container中的keyedTag持有fragment引用 导致fragment不能回收
        if(mContainerId != 0){
            ViewGroup container = mTransactionDelegate.findContainerByIdDetach(mFragment,mContainerId);
            if(container != null){
                container.setTag(androidx.fragment.R.id.visible_removing_fragment_view_tag,null);
            }
        }
    }
